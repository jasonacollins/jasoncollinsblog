---
title: 'Using large language models as an academic June 2024 edition'
author: "Jason Collins"
date: 2024-06-28 09:00:00+10:00
draft: true
aliases:
...
I use multiple tools, switching between them for tasks and comparing their responses.

As an academic, I gain free access to [Github Copilot](https://github.com/features/copilot). (It's only \$10 a month otherwise.) CoPilot is badged as "your coding companion", providing code suggestions in coding environments. But while badged this way, CoPilot also provides text suggestions more broadly.
...
I also work with [ChatGPT Plus](https://chat.openai.com/) and [Claude Pro](https://www.anthropic.com/claude) open in a browser. I subscribe to both: it is worth it for the superior tools.

I run some other models locally, more to get a feel for them than for their utility. I use [Ollama](https://ollama.com/), and most of my experimentation to date has been with [Llama 3](https://ollama.com/library/llama3). I use these models either via the command line or using the CoPilot chat plugin in [Obsidian].

I also subscribe to [Eleven] for it's AI voice capabilities. More on that below.

I don't use Bing a lot, except for images. I like that it still gives me 4 images for every prompt and for some reason I get closer to what I want from Bing than from ChatGPT.

If CoPilot isn't giving me what I want, ChatGPT often will. If I can frame my requirement as a standalone question, ChatGPT tends to do a better job. But when working on a document, CoPilot's suggestions are normally better (unless I paste the whole document into ChatGPT).

## Thinking

I am increasingly using Claude and ChatGPT as an interactive [rubber duck](https://en.wikipedia.org/wiki/Rubber_duck_debugging). When thinking through something new, I'll often state my thoughts and ask for comment.

After reading an article and forming my own views on its message, strengths and weaknesses, I'll ask Claude or ChatGPT. 

When I'm having trouble getting a sentence or paragraph right, I'll often give hand it over and ask for 10 possible rewrites. I'll almost never take a rewrite as provided, but it often gives me a word or turn of phrases that does the job.

If I have a technical question, I'll ask before googling.
...
My typical approach has been to write a comment, let CoPilot do the first cut and then tweak the code until it is in shape.

That tweaking isn't manual tweaking either. If there is an error or problem that I can't resolve, I'll use CoPilot chat to get a solution. The chat function is a major 
...
Here are two older coding applications that I was particularly excited about.
...
When I wanted to generate a chart showing a [probability weighting function](https://behaviouraleconomics.jasoncollins.blog/prospect-theory/probability-weighting) from prospect theory, I created an R code block and typed a comment:
...
My second example relates to a proposed reproduction of the results in Berkeley Dietvorst's [Algorithm aversion: People erroneously avoid algorithms after seeing them err](https://doi.org/10.1037/xge0000033). Data and the code for analysis was [available on ResearchBox](ADD%20LINK).
...
When I wrote my last post, AI-voiced lectures the future. It's not a core part of my subject delivery.

Last year I decided to move the subject out of the 1950s and make it a combination of some [short pre-recorded videos](https://www.youtube.com/@jasonacollins), interactive online seminars and tutorials. To produce the videos I wrote scripts, which also form the [subject notes](https://behaviouraleconomics.jasoncollins.blog). I read the notes accompanied by slides. I'm a hopeless off-the-cuff speaker, so for anything that's going to be recorded and treated as a long-term asset, I'm scripting.
...
I experimented with [Murf.ai](https://murf.ai), [Speechify](https://speechify.com), [play.ht](https://play.ht/) and [Eleven](https://beta.elevenlabs.io). Eleven has the most realistic voices but doesn't yet appear to have an easy way to create video using slides. If they introduce that, I'm fully on board. Murf.ai allows me to pair slides and text to create videos, but the voices still sound a bit robotic. But with the ability to pace their speech, Murf.ai is already a better option than me. And they'll only get better.
...
play.ht and Eleven have the ability to "clone" your voice, but neither can handle non-American accents. The rhythm of my cloned voice (maybe my natural rhythm) also seemed a bit clunky.

Eleven has the ability to do a professional clone of a voice. I did that, and it sounds just like me, even with the Australian accent. Unfortunately, it comes with my faults, particularly speaking too fast. Eleven doesn't have an option to slow down the speaking outside of putting a mountain of dashes through the text. So, for now, I'm using one of their Australian voices.
