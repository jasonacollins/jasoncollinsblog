...
I have sympathy for this critique. However, Marcus didn't convince me at all, and the weakness of his arguments left me wondering if I should update in the other direction.

The prime reason for this is while the general critique might hold, Marcus rarely nailed it when it came to the specifics. As a start, Marcus largely straw-manned the specific evolutionary psychology arguments. Each time he picked a human trait to critique, the arguments were vague and lacked detail. For example, in the discussion of whether mental illness could have an evolutionary advantage (thus explaining its ongoing presence), he refers to:
...
He also doesn't often establish that the problem is a 'kluge'. Most chapters end with a hand-wavy statement about how a more recent deliberative system is bolted onto a reflexive ancient system (the System 1 - System 2 dichotomy made famous by Kahenman).

Marcus also has an interesting definition of optimality, which largely consists of what he believes he could build with a computer. For instance, the benchmark against which our fallible memories is compared is "postal-code memory", where a computer assigns an address to each memory, and when it needs to access it it simply goes there. He states that "Having postal-code memory would have been terrifically useful for us, but evolution never discovered the
right part of the mountain range."

This benchmark of perfection, however, does not acknowledge any of the trade-offs, trade-offs that have become apparent in the world of generative AI. Generative AI that simply memorises it useless. It overfits the data. You want generative AI that generalises.

There is evidence

A discussion of trade-offs, for one, would be interesting.
...
In Chapter 4, he discusses how we make choices. Gigerenzer? Trade-offs.
