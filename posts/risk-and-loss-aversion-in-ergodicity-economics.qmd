...
In the simulations I conducted of the additive passive day, `r round(addSimRange[1], 0)`% of the simulations breach that zero lower bound. `r round(addSimRange[2], 0)`% breach the 5000kr upper bound (some of the same paths that went below zero), leaving only `r round(addSimRange[3], 0)`% of the trials that could be provided to subjects on the passive day. In contrast, passive multiplicative paths could not be excluded for going below zero, despite providing a hair-raising ride. Around `r round(multiSimRange1[2], 0)`% of the passive multiplicative paths that I simulated involve wealth dropping to less than 1kr (a 99.9% loss). Then at the top end, `r round(multiSimRange[2], 0)`% of the passive multiplicative paths went above \$5000kr, leading to their exclusion.
